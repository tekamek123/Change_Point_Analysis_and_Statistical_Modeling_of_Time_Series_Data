"""
Exploratory Data Analysis for Brent Oil Prices (Simplified Version)
This script performs initial EDA on Brent oil price data and generates insights.
"""
...
def perform_simple_stationarity_tests(price_df):
    """Perform basic stationarity tests on price data."""
    print(f"\n8. STATIONARITY ANALYSIS")
    
    # Test on log returns
    price_df['Log_Return'] = np.log(price_df['Price']).diff()
    log_return_series = price_df['Log_Return'].dropna()
    
    # Basic stationarity check - mean reversion test
    mean_return = log_return_series.mean()
    print(f"   - Mean Log Return: {mean_return:.6f}")
    print(f"   - Standard Deviation: {log_return_series.std():.6f}")
    
    # Simple autocorrelation test
    autocorr_lag1 = log_return_series.autocorr(lag=1)
    autocorr_lag5 = log_return_series.autocorr(lag=5)
    autocorr_lag10 = log_return_series.autocorr(lag=10)
    
    print(f"   - Autocorrelation Lag 1: {autocorr_lag1:.4f}")
    print(f"   - Autocorrelation Lag 5: {autocorr_lag5:.4f}")
    print(f"   - Autocorrelation Lag 10: {autocorr_lag10:.4f}")
    
    # Volatility clustering check
    squared_returns = log_return_series ** 2
    vol_clustering = squared_returns.autocorr(lag=1)
    print(f"   - Volatility Clustering (Autocorr of Squared Returns): {vol_clustering:.4f}")
    
    # Interpretation
    if abs(autocorr_lag1) < 0.1:
        print("   - Interpretation: Low autocorrelation suggests weak mean reversion")
    else:
        print("   - Interpretation: High autocorrelation suggests strong momentum effects")
    
    if vol_clustering > 0.2:
        print("   - Volatility clustering is significant (suggests GARCH effects)")
    else:
        print("   - Volatility clustering is moderate")
    
    return {
        'mean_return': mean_return,
        'std_return': log_return_series.std(),
        'autocorr_1': autocorr_lag1,
        'autocorr_10': autocorr_lag10,
...
def generate_eda_report(price_df, events_df, impact_df, stationarity_results=None):
...
"""
    if stationarity_results:
        mean_ret = f"{stationarity_results['mean_return']:.6f}"
        std_ret = f"{stationarity_results['std_return']:.6f}"
        autocorr_1 = f"{stationarity_results['autocorr_1']:.4f}"
        autocorr_10 = f"{stationarity_results['autocorr_10']:.4f}"
        vol_cluster = f"{stationarity_results['vol_clustering']:.4f}"
    else:
        mean_ret = "N/A"
        std_ret = "N/A"
        autocorr_1 = "N/A"
    ...
    report_content += f"""
Mean Log Return: {mean_ret}
Standard Deviation of Returns: {std_ret}
Autocorrelation Lag 1: {autocorr_1}
Autocorrelation Lag 10: {autocorr_10}
Volatility Clustering: {vol_cluster}
...
7. RECOMMENDATIONS FOR CHANGE POINT ANALYSIS
============================================
...
def main():
    ...
    stationarity_results = perform_simple_stationarity_tests(price_df)
    ...
    # Generate report
    report = generate_eda_report(price_df, events_df, impact_df, stationarity_results)
